...
public class TestingPanel extends ProtectedPanel {
	...
	protected void onInitialize() {
		...
		Form<Object> form2 = new Form<Object>("formTabella");
		...
		columns.add(new AbstractColumn<Persona, String>(new Model<String>("cancella")) {
			private static final long serialVersionUID = 1L;

			@Override
			public void populateItem(Item<ICellPopulator<Persona>> cellItem, String componentId,
					final IModel<Persona> rowModel) {
				cellItem.add(new AjaxButton(componentId) {
					...
					protected void onSubmit(AjaxRequestTarget target, Form<?> form) {
						...
						modaleCancella.setContent(new AvvisoCancellazionePanel("content", "Sicuro?") {
							private static final long serialVersionUID = 1L;

							@Override
							protected void onAnswerYes(AjaxRequestTarget target) {
								getPersone().remove(rowModel.getObject());

								if (getPersone().isEmpty())
									personeTable.setVisible(false);
		...
		personeC = new CDropDownChoice<Persona>("personaCh", true, DropDownChoiceNull.EMPTY, personaModel,
				getPersone());
		final AjaxButton btnLabel = new AjaxButton("btnLabel") {
			@Override
			protected void onSubmit(AjaxRequestTarget target, Form<?> form) {
		...
		btnLabel.setOutputMarkupId(true);
		form2.add(btnLabel);
		form2.add(labelTxt);
		form2.add(personeC);
			...
			final AjaxButton button = new AjaxButton("inserisci") {
				...
				*/
				...
				protected void onSubmit(AjaxRequestTarget target, Form<?> form) {
					...
					getPersoneTable().setVisible(true);
	...
	public abstract class AvvisoCancellazionePanel extends AbstractConfirmPanel {

		// private final String domanda;

		public AvvisoCancellazionePanel(String id, String domanda) {
			super(id);
